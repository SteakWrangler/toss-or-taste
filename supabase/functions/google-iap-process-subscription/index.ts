...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[GOOGLE-IAP-PROCESS-SUBSCRIPTION] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    logStep("Google Play IAP subscription processing started");
    ...
    logStep("Processing Google Play IAP subscription", { userId: user.id, productId, orderId });
    ...
    // Check if payment is complete (paymentState = 1 means paid)
    if (validationResult.paymentState !== 1) {
      await supabaseClient.from("google_play_transactions").insert({
        ...
        purchase_date: new Date(parseInt(validationResult.startTimeMillis)).toISOString(),
        validation_status: 'invalid',
        processed: false
      });
      throw new Error(`Payment not complete. Payment state: ${validationResult.paymentState}`);
    }

    const subscriptionType = productId.includes('monthly') ? 'monthly' : 'annual';
    const expiresDate = new Date(parseInt(validationResult.expiryTimeMillis));
    const autoRenewStatus = validationResult.autoRenewing;
    ...
    logStep("Google Play IAP subscription activated successfully", {
      userId: user.id,
      subscriptionType,
    ...
    logStep("ERROR in google-iap-process-subscription", { message: errorMessage });
